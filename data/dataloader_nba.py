...
class NBADataset(Dataset):
    ...
    def __init__(
        ...
        if training:
            data_root = './data/files/SinD_20_train_light_t.npy'
        else:
            data_root = './data/files/SinD_20_test_light_t.npy'
        ...
        self.traj_abs = torch.from_numpy(self.trajs).type(torch.float) #original, transfer to tensor
        #print(self.traj_abs[1,1,1,:])
        self.traj_norm = torch.from_numpy(self.trajs-self.trajs[:,self.obs_len-1:self.obs_len]).type(torch.float) #normalization, set time t to 0
        #print(self.traj_norm[1,1,1,:])
        self.traj_abs = self.traj_abs.permute(0,2,1,3)
        self.traj_norm = self.traj_norm.permute(0,2,1,3)
        self.actor_num = self.traj_abs.shape[1] #actor number
    ...
    def __getitem__(self, index):
        ...
        pre_motion_mask = torch.ones(self.actor_num,self.obs_len)
        fut_motion_mask = torch.ones(self.actor_num,self.pred_len)
        out = [
