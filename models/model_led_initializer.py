...
from models.layers import MLP, social_transformer, st_encoder, Residual, SELayer

class LEDInitializer(nn.Module):
	#changing the mode, with or without light and acceleration
	def __init__(self, t_h: int=10, d_h: int=6, t_f: int=20, d_f: int=2, k_pred: int=20):
	# def __init__(self, t_h: int=10, d_h: int=10, t_f: int=20, d_f: int=2, k_pred: int=20):
		'''
		Parameters
		----
	...
	def forward(self, x, mask=None):
		...
		x: batch size, t_p, 6
		'''
		#turn on or off the resnet and se module
		# x = self.resnet(x)
		# x = self.se(x)
